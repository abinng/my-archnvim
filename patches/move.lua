...
local function move(opts)
  ...
  local function scoring_function(start_, match)
    ...
    local score = 0
    if node.byte_range then
      local sb, eb = node:byte_range()
      score = start_ and sb or eb
    elseif node.range then
      local sr, sc, er, ec = node:range()
      score = start_ and (sr * 100000 + sc) or (er * 100000 + ec)
    end
  ...
  for _ = 1, vim.v.count1 do
    ...
    local target_node = best_match and unwrap_node(best_match.node)
    -- 边界保护：若找不到下一个节点（如已到达文件末尾），静默不执行，避免传空崩溃
    if target_node then
      ts_utils.goto_node(target_node, not best_start, not config.set_jumps)
    end
  end
end
